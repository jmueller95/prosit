...
###Load training data
training_data_path = "/root/training/IM_CID_35_train_1_preprocessed.csv"
...
msms_model_dir = "/root/training/IMA_msms/"
...
#no_partitions = 10
#partition_size = ceil(len(training_df)/no_partitions)

#Compile the models once, and then call fit several times
training.IMA_compile_model(iRT_model, iRT_config)
training.IMA_compile_model(msms_model, msms_config)

#for i in range(no_partitions):
#print("Training Session {} of {}".format(i+1,no_partitions))
#training_df_subset = training_df.iloc[i*partition_size:(i+1)*partition_size]
...
msms_history = training.IMA_train(training_loaded, msms_model, msms_config, msms_callbacks)
